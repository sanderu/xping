...
/*
 * Remove a target (prb) from the hash table if the target is the currently
 * active for the address. Secondly first target (t1) which was duplicate
 * of target (prb) is activated, and other duplicates will refer of that
 * one (prb) instead.
 */
void
